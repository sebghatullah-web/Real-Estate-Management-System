...
// گرفتن اطلاعات نمره
$stmt = $conn->prepare("SELECT * FROM plots_600 WHERE id=?");
...
if ($plot['status'] != 'available') {
    echo "<div class='alert alert-danger m-3'>
          ...
          <a href='plots_600.php' class='btn btn-primary m-3'>بازگشت</a>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['sell_plot'])) {
    ...
    if ($exists) {
        $update = $conn->prepare("UPDATE plots_600 SET status='sold', customer_id=? WHERE id=?");
