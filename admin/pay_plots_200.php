...
if (!$plot_id) {
    header("Location: list_plots_200.php");
    exit;
}
...
$pay_sql = "SELECT IFNULL(SUM(amount),0) AS paid_amount FROM pay_200 WHERE plot_id=?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['amount'])) {
    ...
    if ($amount <= $remaining && $amount > 0) {
        $insert = $conn->prepare("INSERT INTO pay_200 (plot_id, customer_id, amount) VALUES (?, ?, ?)");
        $insert->bind_param("iid", $plot_id, $plot['customer_id'], $amount);
        $insert->execute();

        // بعد از ثبت پرداخت، چاپ انوایس
        header("Location: invoice_200.php?plot_id=$plot_id&amount=$amount");
        exit;
    } else {
  ...
  <?php if ($remaining > 0): ?>
    <form method="post" class="row g-3">
      ...
      <div class="col-12">
        ...
        <a href="list_plots_200.php" class="btn btn-secondary">بازگشت</a>
      </div>
    </form>
  <?php else: ?>
